...
\Abstract{%
  ...
  (LMMs). The two main extensions provided by GLMMs is that (1) they
  allow for the conditional distribution of the response given the
  random effects to be something other than a Gaussian (e.g. binomial,
  Poisson) and (2) the conditional mean depends upon a linear
  combination of fixed and random effects through an inverse link
  function. The conditional mode of the random effects, given the
  observed data is determined through penalized iteratively reweighted
  least squares (PIRLS). We optimize the Laplace approximation at the
  conditional mode to determine the MLE's of the parameters.  In some
  simple cases, a more accurate approximation, adaptive Gauss-Hermite
  quadrature (AGQ), can be used instead, at the expense of greater
...
range of mixed-effects models.  One of the main advantages of
\pkg{lme4} over its predecessor, \pkg{nlme}, is that it can be used to
fit both linear mixed models (LMMs) and generalized linear mixed
models (GLMMs), which combine the flexibility of LMMs and generalized
...
$\mc Y$ has a distribution in the exponential family (e.g. normal,
...
scalar link function must be invertible over its range.  The
...
In the GLMM case, the mean of the exponential family distribution of
...
conditional mean of $\mc Y$ given that $\mc B = \bm b$, writen
...
to the GLM case, the mapping from the conditional mean, $\bm\mu_{\mc
  Y|\mc B = \bm b}$, to the linear predictor, $\bm\eta$, 
\begin{equation}
  \label{eq:linkfunii}
...
depends on a vector of covariance parameters, $\theta$.

The optimization routines of \pkg{lme4} never actually compute
$\Sigma_\theta$ directly, and instead use the covariance factor,
$\bm\Lambda_\theta$, which is a matrix squareroot of $\Sigma_\theta$,
\begin{equation}
  \label{eq:GLMMSigma}
...
\end{equation}
where the spherized random effects vector, $\bm u$, is a realization
...
<<cbppPlot,echo=FALSE,dependson="newcbpp",fig.cap="Incidence (proportion of cows becoming seropositive per observation period) vs. period. Colours show treatment category for each herd; point sizes and line widths show size (seronegative cows at the start of each period) and average herd size respectively.",fig.scap="Incidence">>=
...
g0 +  geom_point(aes(size = size),alpha=0.4) +
    geom_line(aes(group=herd,lwd=avg_size),alpha=0.3)
...
in computing the hat matrix. \bmb{can we fix this?}
...
p4 <- qqmath(gm1,"Quantile-quantile plot",type=c("p","r"))
grid.arrange(p1,p2,p3,p4,nrow=2)
@    

The \code{ranef()} accessor extracts the conditional modes
\bmb{can we call these ``estimates''??}; the argument
...
is standardly referred to as the inverse link function.
